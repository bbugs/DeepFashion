...
% sus: Size of full parameter vector: 3148600 or 3008600 with one word 
...
if params.sus_toy == false
    train_split = getDatasetFold(params, 'train');
    fprintf('getting validation fold...\n');
    val_split = getDatasetFold(params, 'val');
else
    train_split = getDatasetFold(params, 'val');
    fprintf('getting validation fold...\n');
    val_split = getDatasetFold(params, 'test');   
end
Ni = length(train_split.Img);  % 4000 for toy
Ns = length(train_split.Sent);  % 29122 for toy
...
while iter <= maxiters
    ...
    rp = randperm(Ni);
    rpb = rp(1:params.batch_size);  % sus batch_size 35
